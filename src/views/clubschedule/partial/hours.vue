...
export default {
   computed: {
    ...
    getTitle(){
      return this.clubphours.length > 0 ? this.clubphours[0].title + ' Programı' : '';
    }
    },
  components: {
    DTime
    ...
    hourData: {
      ...
      async created(){
        const atp_id = this.$route.params.id;
         if (atp_id && atp_id > 0) {
            let result = await this.$store.dispatch("fetchClubPHours", {where: "club_p_type_id", id: atp_id});
            if(!result){
              this.$router.go(-1);
            }
            this.atp_id = atp_id;
  ...
  methods: {
      getAction(item, action){
        ...
        .then(res => {
          if (res.value) {
            this.$store.dispatch("deleteClubPHours", item);
          }
        });
      },
    async save() {
      if (this.$v) {
        if (this.$v.$invalid) {
          ...
          const result = await this.$store.dispatch("createClubPHours", payload)
          if (result) {
            await this.$store.dispatch("fetchClubPHours", {where: "club_p_type_id", id: this.atp_id});
