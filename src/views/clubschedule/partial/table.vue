...
export default {
  ...
  methods: {
    ...
    handleCopy(day, hour){
       this.data = this.clubpcontents.find(apc=>apc.club_day_id === day.id && apc.club_hour_id === hour.id);
        this.$toast.success('Kopyalandı', {
                position: 'top'
        })
    },
    async handlePaste(day, hour){
        let data = this.clubpcontents.find(apc=>apc.club_day_id === day.id && apc.club_hour_id === hour.id);
         if(data && data.id){
             this.data = {
                 ...this.data,
                 type: 1
    ...
    async handleDelete(day, hour){
        ...
        .then(async res => {
          if (res.value) {
                const data = await this.clubpcontents.find(apc=>apc.club_day_id === day.id && apc.club_hour_id === hour.id);
                await this.$store.dispatch("deleteClubPContents", data);
          }
        });
    },
    handleHour(day, hour) {
