      ...
      <vuetable
       ...
       >
          <template slot="pimage" scope="props">
                    ...
                    <!-- <button v-if="schedule" class="btn btn-success btn-sm" @click="onViewSchedule(props.rowData, props.rowIndex)">Görüntüle</button> -->
                      ...
                      <div v-if="!assign" class="custom-actions">
                        <button
                          ...
                          @click="onSelectData(props.rowData)"
                        >
                          Detay
                        </button>
                      </div>
...
export default {
  ...
  methods: {
    ...
    async onFetchApi(apiUrl, httpOptions) {
         var data;
         data = this.$store.dispatch("fetchEmployeeList", {
          httpOpt: httpOptions,
          ...this.query
        });
        data.then(res => {});
