       ...
       >
        ...
        <template slot="actions" scope="props">
          <div class="custom-actions">
            ...
            <button v-if="main == 'main'" class="btn btn-danger btn-sm" @click="onRemoveData(props.rowData)">Sil</button>
          </div>
        </template>
      </vuetable>
...
export default {
  ...
  methods: {
    ...
    onRemoveData(student){
      appPlugin.showConfirm("Bu işlemi onaylarsanız, öğrenciyi ve ilişkili verilerini kaybedeceksiniz. Yine de onaylıyor musunuz ?").then(async res=>{
        if(res.value){
    ...
    onSelectData(data) {
      ...
      // console.log(data.file ? data.file.path:"");
      let model = {
        id: data.id,
        name: data.s_name,
        email: data.s_email,
    ...
    async onFetchApi(apiUrl, httpOptions) {
           var data;
         data = this.$store.dispatch("fetchStudentList", {
          httpOpt: httpOptions,
          ...this.query
        });
        data.then(res => {});
