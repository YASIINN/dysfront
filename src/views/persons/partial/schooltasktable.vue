      ...
      <div class="col-md-12 col-sm-12 col-lg-12">
        ...
        <div class="table-responsive">
          ...
          <vuetable
            ...
            :fields="$store.getters.getUserSchoolHeader"
            ...
            <template slot="actions" scope="props">
              <div class="custom-actions">
                <v-tooltip-button
                  ...
                  :title="'Sil'"
                  :btnColor="'red'"
                  @onClick="onDeleteUserSchool(props.rowData, props.rowIndex)"
                ></v-tooltip-button>
              </div>
            </template>
          </vuetable>
    ...
    methods: {
      exportExcel () {
        ...
        if (data.length < 1) {
          ...
          let keys = ['sCode', 'sName']
          const header = ['Okul Kodu', 'Okul Adı']
      ...
      onDeleteUserSchool (item, i) {
        Swal.fire({
          ...
          if (res.value) {
            ...
            this.$store
              .dispatch('deleteUserSchool', {
                deleted: item,
                index: i,
                userid: item.pivot.users_id,
                schoolid: item.pivot.school_id,
                surlparse: appPlugin.urlParse(
                  'users_id=' +
                  item.pivot.users_id +
                  '& school_id=' +
                  item.pivot.school_id
                ),
      ...
      onFetchApi (apiUrl, httpOptions) {
        ...
        if (this.txt.trim() != '') {
          if (isNaN(+this.$route.params.id) == false) {
            data = this.$store.dispatch('getUserSchoolU', {
