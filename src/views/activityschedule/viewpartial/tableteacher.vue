...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      apteachercontents: "apteachercontents",
    }),
    getTitle(){
  ...
  methods: {
    getLesson(day, hour) {
      const data = this.apteachercontents.find(apc=>apc.activity_day_id === day.id && apc.activity_hour_id === hour.id);
      return data && data.slesson ? data.slesson.name: "";
    },
     getGrade(day, hour) {
      const data = this.apteachercontents.find(apc=>apc.activity_day_id === day.id && apc.activity_hour_id === hour.id);
      return data && data.slesson ? data.sgrade.name: "";
    },
